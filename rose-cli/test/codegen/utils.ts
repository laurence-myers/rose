...
describe(`utils`, () => {
	describe(`mergeImports()`, () => {
		it(`should merge duplicate named imports`, async () => {
			// Set up
			const imports1 = [imp([namedImport("Foo")], "bar")];
			const imports2 = [imp([namedImport("Foo")], "bar")];

			// Execute
			const result = mergeImports(imports1, imports2);

			// Verify
			assert.deepEqual(result, [imp([namedImport("Foo")], "bar")]);
		});

		it(`should NOT merge duplicate named imports when passed the same array`, async () => {
			// Set up
			const imports1 = [
				imp([namedImport("Foo")], "bar"),
				imp([namedImport("Foo")], "bar"),
			];
		...
		it(`should merge duplicate "all" imports`, async () => {
			// Set up
			const imports1 = [impAll("../bar", "bar")];
			const imports2 = [impAll("../bar", "bar")];

			// Execute
			const result = mergeImports(imports1, imports2);

			// Verify
			assert.deepEqual(result, [impAll("../bar", "bar")]);
		});
