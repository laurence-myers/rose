...
const typeMapObjectSchema = Type.Dict(
	Type.Union([typeMapEntrySchema, stringSchema])
);
...
export const configSchema = Type.Object({
	...
	types: Type.Optional(
		Type.Object({
			global: Type.Optional(typeMapObjectSchema),
			columns: Type.Optional(typeMapObjectSchema),
		})
	),
});
...
export function convertTypeMapObjectToMap(
	obj: TypeMapObject,
	outputMap: Map<string, TypeMapEntry> = new Map()
...
export function mergeConfigWithDefaults(
	...
	let globalTypes: Map<string, TypeMapEntry> = new Map(
		defaultPostgresTypeMap.entries()
	);
	globalTypes = convertTypeMapObjectToMap(
		config?.types?.global || {},
		globalTypes
	);
	...
	return {
		schema: "public",
		ignoredTables: defaultIgnoredTables.concat(
			(config.ignore || []).map((name) => name.toLowerCase())
		),
		types: {
			global: globalTypes,
			columns: columnTypes,
...
export async function parseConfig(configFileName: string): Promise<CliConfig> {
	...
	try {
		...
		throw new ConfigError(
			`Could not read from config file "${configFileName}", make sure it is well-formed JSON.`
		);
	}
