	...
	describe(`functions`, () => {
		const cases = new Map<string, SimpleCaseEntry>([
			[
				num_nonnulls.name,
				{
					astNode: num_nonnulls(constant('foo'), null_(), param(() => 1)),
			...
			[
				num_nulls.name,
				{
					astNode: num_nulls(constant('foo'), null_(), param(() => 1)),
	...
	describe(`predicates`, () => {
		const cases = new Map<string, SimpleCaseEntry>([
			...
			[
				...
				{
					astNode: betweenSymmetric(QOrders.amount.col(), constant(1), constant(5)),
