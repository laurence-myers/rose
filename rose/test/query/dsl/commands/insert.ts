...
import {
	...
	doUpdate,
	targetIndex,
} from "../../../../src/query/dsl/onConflict";
import {
	char_length,
	upper,
} from "../../../../src/query/dsl/postgresql/string";
import {
	greaterThanOrEqual,
	lessThanOrEqual,
} from "../../../../src/query/dsl/postgresql/comparison";
...
describe(`INSERT commands`, () => {
	it(`supports inserting a single row`, function () {
		...
		const query = withParams<Params>()((p) =>
			insert<TUsers, InsertRow>(QUsers)
				.insert({
	...
	it(`supports inserting a single row where casing differs between the object and the columns`, function () {
		...
		const query = insert<TUsers, InsertRow>(QUsers).insert({
			id: p.id,
			name: p.name,
			deletedAt: p.deletedAt,
			locationId: p.locationId,
	...
	it(`can omit nullable fields`, function () {
		...
		const query = insert<TUsers, InsertRow>(QUsers).insert(insertRow);
	...
	it(`ignores properties that have a value of "undefined"`, function () {
		// Set up
		const query = withParams<Pick<UsersInsertRow, "name" | "locationId">>()(
			(p) =>
				insert(QUsers)
					.insert({
						id: default_(),
			...
			(err: { message?: string } | undefined) => {
				return (
					...
					err.message.startsWith(
						`Inserted row columns doesn't match the expected columns`
					)
				);
			}
		);
	...
	it(`supports inserting from a sub-query, using alias to specify column name`, function () {
		...
		const query = insert(QUsers).insertFromQuery(
			subSelect(alias(col(QLocations.id), "locationId"))
	...
	it(`supports inserting from a sub-query with a nested sub-query`, function () {
		// Setup
		function projectRoleLookupCriteria(
			p: ParamsProxy<
				Pick<ProjectRoleRow, "memberId" | "role"> &
					Pick<ProjectRow, "shortCode">
			>
		) {
			return and(
				//
		...
		const query = withParams<
			Pick<ProjectRoleRow, "memberId" | "role"> & Pick<ProjectRow, "shortCode">
		>()((p) =>
			insert(QProjectRole)
				.insertFromQuery(
					subSelect(now(), p.memberId, null_(), QProject.projectId, p.role)
						.join(
							leftJoin(QProject).on(
								and(
									QProject.shortCode.eq(p.shortCode),
	...
	it(`supports returning values`, () => {
		...
		const query = insertFromObject<TUsers, InsertRow>(
			QUsers,
			insertRow
		).returning({
			newUserId: QUsers.id,
		...
		describe(`do update`, () => {
			it(`can pass column references`, () => {
				...
				const actual = insert(QUsers)
					...
					.onConflict(
						doUpdate()
							.onColumns([QUsers.name, QUsers.locationId])
							.set(QUsers, {
				...
				const actual = insert(QUsersSC)
					...
					.onConflict(
						doUpdate()
							.onIndexes(
								[
									targetIndex(upper(QUsers.name.scol()), {
										opclass: "text_pattern_ops",
										collation: "en_US",
									}),
								],
								lessThanOrEqual(char_length(QUsers.name.scol()), constant(10))
							)
							.set(QUsersSC, {
								name: constant("Fred 2"),
			...
			it(`can pass "where" clause in the conflict target, converting ColumnReferenceNodes to SimpleColumnReferenceNodes`, () => {
				...
				const actual = insert(QUsers)
					...
					.onConflict(
						doUpdate()
							.onColumns(
								[QUsers.name, QUsers.locationId],
								QUsers.deletedAt.isNull()
							)
							.set(QUsers, {
								name: constant("Fred 2"),
							})
